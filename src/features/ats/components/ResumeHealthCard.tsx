...
function ResumeHealthCardComponent({ analysis }: ResumeHealthCardProps) {
  return (
    <section className="surface-card grid gap-6 p-6 lg:grid-cols-[auto_1fr]">
      <CircularScore score={analysis.score} />
      <div className="flex flex-col justify-center">
        <p className="text-sm font-medium text-primary">Resume Health</p>
        <h2 className="mt-2 text-3xl font-semibold">{getHealthLabel(analysis.score)}</h2>
        ...
        <dl className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-border bg-muted/20 p-4">
            ...
            <dd className="mt-1 text-2xl font-semibold">{analysis.criticalIssues.length}</dd>
          </div>
          <div className="rounded-xl border border-border bg-muted/20 p-4">
            <dt className="text-xs text-muted-foreground">Warnings</dt>
            <dd className="mt-1 text-2xl font-semibold">{analysis.warnings.length}</dd>
          </div>
          <div className="rounded-xl border border-border bg-muted/20 p-4">
            <dt className="text-xs text-muted-foreground">Passed checks</dt>
            <dd className="mt-1 text-2xl font-semibold">{analysis.passedChecks.length}</dd>
          </div>
        </dl>
      </div>
    </section>
